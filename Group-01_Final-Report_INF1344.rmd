...
ggplot(data_2023, aes(y=broadband)) + 
  geom_boxplot() + 
  labs(title = "Broadband Subscribers per 100 people, in 2023",
       y="Subscribers per 100 people")
```
...
We need to test our model against the assumptions for multiple linear regression
1. Linearity
...
## Comparison since UN declaration
```{r}
# Reduce the dataframe to the years of interest
df_before_after <- merged_data %>% filter(year==2011|year==2023) 
glimpse(df_before_after) 

# Get a list of all of the years in the HDI dataset
ggplot(df_before_after, aes(y=broadband)) +
geom_boxplot() +
    facet_wrap(~year, scales = 'fixed') # Gives the plots the same scales

# Conducting a t-test to confirm that the number of subscribers is sigificantly different
t.test(data_2011$broadband, data_2023$broadband)
