...
It might be prudent to not include the year 2023 in our analysis as it has a lot of missing values. In any case, we will remove the rows displayed above with empty values.

``` {r echo=FALSE, message=FALSE, warning=FALSE}
hdi_clean <- hdi %>% drop_na()
broadband_clean <- broadband %>% drop_na
gdp_clean <- gdp %>% drop_na
```
