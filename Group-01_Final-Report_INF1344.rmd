...
Let's review the structure of the HDI dataframe.
```{r}
str(hdi)
```
...
Let's review the structure of the broadband subscriber dataframe.
```{r}
str(broadband)
```
...
Let's inspect the structure of the GDP dataframe for the years of interest (2010-2023).
```{r}
...
```
Let's look at this row for the missing value
```{r}
...
For each, let's forward fill with the nearest previous year as an approximation
```{r}
...
ggplot(data_2022_standardized, aes(x = sqrt_broadband, y = hdi)) +
  ...
  labs(title = "Square root Broadband vs HDI in 2022",
...
multi_reg_2011 <- lm (hdi ~ sqrt_broadband + log_gdp, data = data_2011_standardized)
...
dwtest(multi_reg)
```
??? Not sure here - While we failed to reject the null hypothesis that autocorrelation is greater than zero, as the value is not less than 1 or greater than 3, we are less concerned about autocorrelation.
...
```
The residuals are fairly normal in the first quantile.
...
```
The residuals do not cluster evenly. There is some heteroscedascity.
...
data_2022_standardized <- na.omit(data_2022_standardized) %>%
  mutate(
    fitted_hdi = fitted(multi_reg),
    residuals = resid(multi_reg),
    std_resid = rstandard(multi_reg)
  )
...
data_2011_standardized <- na.omit(data_2011_standardized) %>%
  mutate(
    fitted_hdi = fitted(multi_reg_2011),
    residuals = resid(multi_reg_2011),
    std_resid = rstandard(multi_reg_2011)
  )
...
merged_data  %>% filter(year %in% c('2011','2022')) %>% ggplot(aes(y=broadband)) +
    geom_boxplot() +
    facet_wrap(~year, scales = 'fixed') 
```
This data is skewed and has unequal variances.

## Wilcoxin Rank-Sum Test
Let's test whether there is significant differences between broadband subscriptions between our reference year and 2022. We'll use a non-parametric test because our skewed data doesn't meet the conditions for a t-test.
```{r}
result <- wilcox.test(data_2022$broadband,data_2011$broadband)
result
```
...
```
On average, countries had greater broadband subscription rates in 2022 (M = 16.24, SE=15.23) than in 2011 (M=9.61, SE=11.31).  This difference is significant W=20910, p< 0.05.
...
There is a significant relationship between HDI and broadband subscription rate, r=.90, p<.05.
